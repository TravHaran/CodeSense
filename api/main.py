...
from models import ModelRequest, BatchModelRequest, QueryRequest, BatchQueryRequest, SearchRequest, BatchSearchRequest
...
@app.get('/batchSearch')
async def batchSearch(request: BatchSearchRequest):
    codebases = request.models
    batch_models = ""
    for path in codebases:
        extractor = CodeBaseExtractGithub(path)
    ...
    # get models
    models = json_to_obj(f"database/batch_models/{batch_models}.json")
    response = BatchSearch(models, question).run()
    # save response
    now = str(datetime.now())
